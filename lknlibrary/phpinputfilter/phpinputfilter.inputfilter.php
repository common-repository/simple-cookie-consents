 ...
 * Time: 13:02
 */
class lknInputFilter
{
	/**
	 * A container for lknInputFilter instances.
	 *
	 * @var    array
	 */
	 ...
	 * @var    array
	 */
	public $tagsArray = array("a", "b", "blink", "blockquote", "br", "caption", "center", "col",
		"colgroup", "comment", "em", "font", "h1", "h2", "h3", "h4", "h5", "h6", "hr", "img", "li",
		"marquee", "ol", "p", "pre", "s", "small", "span", "strike", "strong", "sub", "sup", "table",
		"tbody", "td", "tfoot", "th", "thead", "tr", "tt", "u", "ul", "div");
	...
	public $attrArray = array("id",
		...
		"border",
		"bordercolor",
		"bordercolordark", "bordercolorlight", "bottompadding",
		"cellpadding", "cellspacing", "char", "charoff", "cite",
		"clear", "color", "cols", "direction", "face", "font-weight",
		"headers", "height", "href", "hspace", "leftpadding", "loop", "noshade",
		"nowrap", "point-size", "rel", "rev", "rightpadding", "rowspan", "rules",
		"scope", "scrollamount", "scrolldelay", "size", "span", "src", "start", "summary",
		"target", "title", "toppadding", "type", "valign", "value", "vspace", "width", "wrap");
	 ...
	 * @param   integer $xssAuto Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
	 *
	 * @return  lknInputFilter  The lknInputFilter object.
	 *
	 */
	public static function &getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
	{
		...
		if (empty(self::$instances[$sig])) {
			self::$instances[$sig] = new lknInputFilter($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
		}
	 ...
	 * @internal param mixed $source Input string/array-of-string to be 'cleaned'
	 * @return  mixed  'Cleaned' version of input parameter
	 */
	public function clean(&$arr, $name, $def = null, $type = '')
	{
		...
		return $this->_clean($source,$type);
	}
	
	
	public  function _clean($source,$type = ''){
		...
		switch (strtoupper($type)) {
			...
			case 'INTEGER':
				...
				$result = isset($matches[0]) ? (int)$matches[0] : (int)array();
				break;
			
			case 'UINT':
				// Only use the first integer value
				preg_match('/-?[0-9]+/', (string)$source, $matches);
				$result = isset($matches[0]) ? @abs((int)$matches[0]) : @abs((int)array());
				break;
			...
			case 'DOUBLE':
				...
				$result = isset($matches[0]) ? @(float)$matches[0] : (float)array();
				break;
			...
			case 'PATH':
				...
				$result = isset($matches[0]) ? @(string)$matches[0] : (string)('');
				break;
	...
	protected function _decode($source)
	{
		...
		$source=$this->decodeFix($source);
		
		return $source;
	}
	
	private function decodeFix($str){
		$search=array('&uuml;','&ouml;');
		$replace=array('ü','ö');
	 ...
	 * @return  mixed  'Cleaned' version of input parameter
	 */
	public static function filterInput(&$arr, $name, $def = null, $type = ''){
		if($type=='HTML'){
			$filter=lknInputFilter::getInstance( array(), array(), 1, 1);
		}else{
			$filter=lknInputFilter::getInstance();
		}
		
		
		return $filter->clean($arr,$name,$def,$type);
	}
}